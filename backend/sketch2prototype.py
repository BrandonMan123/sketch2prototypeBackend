...
def sketch_to_text_wrapped(sketch):
    json_data = convert_sketch_to_text(sketch)
    return json_data['choices'][0]['message']['content']

def text_to_image_wrapped(text, num_images):
    image_response = convert_text_to_image(text, num_images, response_type="b64_json")
    # print ("image response", image_response.data[0])
    images = [image_response.data[i].b64_json for i in range(len(image_response.data))]
    # print ("Images", images)
    return images
...
def convert_text_to_image(text, num_images, model="dall-e-3", response_type="url"):
